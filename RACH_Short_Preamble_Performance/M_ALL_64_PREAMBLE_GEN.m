function [ZC_64,Ncs,ZC_root,ZC_ALL]=M_ALL_64_PREAMBLE_GEN(zCZC)
...
%This section genrated possible values of cyclic shifts for a given zCZC
zCZC_index=zCZC+1;
...
W=exp((-1j*2*pi)/Lra);
% ZC_ALL= (W.^((k).^3)).*(W.^((root_index_map').*k));
for i=1:138
 ZC_ALL(i,:)=AllTop_seq_gen(Lra,1,i,1);
end
M=M_seq_gen(Lra); %see FUNCTION M_seq_gen
