function [ZC_64,Ncs,ZC_root,ZC_ALL]=ZC_A_64_PREAMBLE_GEN(zCZC)
...
% % M=M_seq_gen(Lra);
% % ZC_ALL=ZC_ALL.*M;
% lambda=0:138; w=0:138; n=0:138; l=1:138;
% W=exp((-1j*2*pi)/Lra);
% All_1_0= (W.^((n+w(1)).^3)).*(W.^(lambda(2).*n));
% % All_1_19= (W.^((n+w(20)).^3)).*(W.^(lambda(2).*n));
% ZC_ALL= ZC_ALL.*((All_1_0).^l(1));

%% NEW ALGO
for i=1:138
 ZC_ALL(i,:)=AllTop_seq_gen(Lra,1,1,i); % see FUNCTION AllTop_seq_gen
end
ZC=exp((-1j.*(1)*pi.*k.*(k+1))/Lra);
ZC_ALL=ZC_ALL.*ZC;
